...
PROJECT_CONF_FILE="../conf/Flyer.xml"


echo "Run Flyer rpc project, name: ${PROJECT_NAME}, path: ${PROJECT_BIN_FILE}"
...
then
  echo "Run Flyer rpc server eror, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
...
then
  echo "Run Flyer rpc error, file: ${PROJECT_CONF_FILE} not exist, please check config file"
  exit -1
fi
...
echo "Start Flyer rpc server ${PROJECT_CONF_FILE} succ"