...
void TinyPBCoder::encode(std::vector<AbstractProtocol::s_ptr>& messsage, TcpBuffer::s_ptr out_buffer) {
    for(auto &i : messsage) {
...
const char* TinyPBCoder::encodeTinyPb(std::shared_ptr<TinyPBProtocol> message, int& len) {
    if(message->m_msg_id.empty()) {
        message->m_msg_id = "00000";
    }
    DEBUGLOG("msg_id = %s", message->m_msg_id.c_str());

    int pk_len = 2 + 24 + message->m_msg_id.length() + message->m_method_name.length() + message->m_pb_data.length();
    ...
    *tmp = TinyPBProtocol::PB_END;
    ...
    DEBUGLOG("encode message [ %s ] success, msg_id=[%s]", message->m_pb_data.c_str(), message->m_msg_id.c_str());
    return buf;
}


// stream in buffer to message obj
void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr>& out_messsage, TcpBuffer::s_ptr in_buffer) {
    while(1) {
        // ergodic buffer, find PB_START
        // get package length to position end; then judge if it is PB_END 
        ...
        if(parse_success) {
            ...
            out_messsage.push_back(message);
        }
    }
}
